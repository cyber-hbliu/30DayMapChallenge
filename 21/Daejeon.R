...
load_builtup_data <- function() {
  l <- rlinks[grepl("40N_120E", rlinks, rlinks)]
...
# Daejeon BOUNDARIES FROM OSM DATA
#--------------------------------------
city <- "Daejeon"

Daejeon_border <- osmdata::getbb(
  city,
...
plot(Daejeon_border, add = T)


# MAKE BUFFER AROUND Daejeon
#----------------------------
get_buffer <- function() {
  Daejeon_cents <- sf::st_centroid(Daejeon_border)
  Daejeon_circle <- sf::st_buffer(
    Daejeon_cents,
  ...
  return(Daejeon_circle)
}

Daejeon_circle <- get_buffer()

# plot
ggplot() +
  geom_sf(
    data = Daejeon_border, color = "#3036ff",
    fill = "transparent", size = 1.5,
    inherit.aes = FALSE
  ) +
  geom_sf(
    data = Daejeon_circle, color = "#e83778",
    fill = "transparent", size = 1.5,
    inherit.aes = FALSE
  ) +
  theme_void() +
...
crop_builtup_data <- function() {
  Daejeon_vect <- terra::vect(Daejeon_circle)
  Daejeon_raster <- terra::crop(builtup_data, Daejeon_vect)
  Daejeon_raster_cropped <- terra::mask(
    Daejeon_raster, Daejeon_vect
  )
  return(Daejeon_raster_cropped)
}

Daejeon_raster_cropped <- crop_builtup_data()
terra::plot(Daejeon_raster_cropped)
...
raster_to_df <- function() {
  Daejeon_df <- terra::as.data.frame(
    Daejeon_raster_cropped,
    xy = T
  )
...
Daejeon_df <- raster_to_df()
head(Daejeon_df)
names(Daejeon_df)[3] <- "value"

# define categorical values
Daejeon_df$cat <- round(Daejeon_df$value, 0)
Daejeon_df$cat <- factor(Daejeon_df$cat,
                        labels = c("No Built-up", "New", "Existing")
)

# 5. GET Daejeon ROADS FROM OSM DATA
#---------------------------------
road_tags <- c(
...
get_osm_roads <- function() {
  bbox <- sf::st_bbox(Daejeon_circle)
  roads <- bbox |>
    opq() |>
    add_osm_feature(
      key = "highway",
...
roads <- get_osm_roads()
Daejeon_roads <- roads$osm_lines |>
  sf::st_set_crs(crsLONGLAT) |>
  sf::st_transform(crs = crsLONGLAT)

ggplot() +
  geom_sf(
    data = Daejeon_circle, fill = "transparent",
...
# 6. CROP Daejeon ROADS WITH BUFFER
#--------------------------------
Daejeon_roads_cropped <- sf::st_intersection(
  Daejeon_roads, Daejeon_circle
)
...
ggplot() +
  ...
  geom_sf(
    data = Daejeon_roads_cropped, fill = "transparent",
    color = "#e83778", inherit.aes = FALSE
  ) +
  theme_void() +
  theme(panel.grid.major = element_line("transparent"))
...
ggsave(
  filename = "/Users/cyberhbliu/Desktop/30daymapchallenge/21 raster/Daejeon_built_up.svg",
